...
%DIF DEL versao_antiga/main-texto-completo.tex   Thu May 18 19:54:27 2023
%DIF ADD versao_nova/main-texto-completo.tex     Thu May 18 19:56:10 2023
...
%DIFDELCMD <     %%%
\DIFdelend Dados do IBGE apontam que de 2013 a 2020 ocorreu o aumento da população de animais de estimação (cães, gatos, aves e répteis) nos domicílios brasileiros, como mostra o gráfico da Figura \ref{fig:grafico_pet}. E ainda que a quantidade de domicílios com animais tenha sofrido uma pequena queda, \citeonline{ibge2013}, \citeonline{pet2021}, o mercado \emph{pet} no Brasil encontra-se em ascensão nos últimos anos.
...
%DIFDELCMD <         \centering
%DIFDELCMD <         \includegraphics{images/grafico_censopet.png}
%DIFDELCMD <         %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Censo população Pet no Brasil, \mbox{%DIFAUXCMD
\citeonline{ibge2013, ibge2018, pet2021}}\hspace{0pt}%DIFAUXCMD
}}
        %DIFAUXCMD
%DIFDELCMD < \label{fig:grafico_pet}
...
\chapter[Metodologias]{Metodologias}
    ...
    \label{cap:funcionalidades}
        ...
        Considerando as \DIFdelbegin \DIFdel{necessidades do dia a dia }\DIFdelend \DIFaddbegin \DIFadd{demandas cotidianas }\DIFaddend de um médico-veterinário, suas atividades \DIFdelbegin \DIFdel{dentro da clínica e partindo do }\DIFdelend \DIFaddbegin \DIFadd{no ambiente clínico e com base no }\DIFaddend estudo da concorrência, serão implementadas as seguintes funcionalidades:
...
\item%DIFAUXCMD
\DIFdel{Validador de }\DIFdelend %DIF >  \item Validador de cadastro profissional para acessar área restrita do sistema, considerando que algumas atividades são de exclusiva responsabilidade de um profissional veterinário em atividade, como pode ser lido nos Quadros \ref{tab:req_func} e \ref{tab:hist_usuario}.
            %DIF >  \item Histórico de aspectos hereditários, permite que o veterinário faça as devidas anotações quando for observado casos de alergia ou intolerância a medicamentos que possuam aspectos genéticos e doenças que podem ser transmitidas geneticamente. Servindo de base para melhor atendimento de um animal que possua consanguinidade ou parentesco com o animal das observações.
        \DIFaddbegin \end{itemize}

        \subsection{\DIFadd{Funcionalidades Futuras}}
        \label{subcap:funcionalidades-futuras}

            \DIFadd{Dentre as possibilidades de futuras funcionalidades, destacam-se a transferência de dados entre diferentes  estabelecimentos que utilizam a CertVet, a digitalização das notificações de aquisição de medicamentos controlados, acompanhada da Nota Fiscal referente à compra do produto, automatizando a entrada desses dados no estoque e possibilitando uma fiscalização mais precisa e eficiente.
            }

            \DIFadd{Além disso, há o validador de }\DIFaddend cadastro profissional para \DIFdelbegin \DIFdel{acessar }\DIFdelend \DIFaddbegin \DIFadd{acesso à }\DIFaddend área restrita do sistema, considerando que \DIFdelbegin \DIFdel{algumas }\DIFdelend \DIFaddbegin \DIFadd{determinadas }\DIFaddend atividades são \DIFdelbegin \DIFdel{de exclusiva }\DIFdelend \DIFaddbegin \DIFadd{exclusivamente }\DIFaddend responsabilidade de um \DIFdelbegin \DIFdel{profissional }\DIFdelend \DIFaddbegin \DIFadd{médico }\DIFaddend veterinário em \DIFdelbegin \DIFdel{atividade, como pode ser lido }\DIFdelend \DIFaddbegin \DIFadd{exercício, como detalhado }\DIFaddend nos Quadros \ref{tab:req_func} e \ref{tab:hist_usuario}. \DIFdelbegin %DIFDELCMD < \item %%%
\DIFdel{Histórico }\DIFdelend \DIFaddbegin \DIFadd{Por fim, o histórico }\DIFaddend de aspectos hereditários \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{que }\DIFaddend permite que o veterinário faça as devidas anotações quando for observado casos de alergia ou intolerância a medicamentos que possuam aspectos genéticos e doenças que podem ser transmitidas geneticamente. Servindo de base para melhor atendimento de um animal que possua consanguinidade ou parentesco com o animal das observações.
        \DIFdelbegin %DIFDELCMD < \end{itemize}
%DIFDELCMD < %%%
\DIFdelend 

        \DIFdelbegin \subsection{\DIFdel{Funcionalidades Futuras}}
            %DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD <             %%%
\DIFdel{Algumas possíveis implementações de funcionalidades futuras são a importação e exportação dos dados entre estabelecimentos que utilizam o mesmo sistema, digitalização das notificações de aquisição de medicamentos controlados com a nota fiscal da compra do produto, automatizando a entrada desses dados no estoque e permitindo a fiscalização.
        }%DIFDELCMD < 

%DIFDELCMD <         %%%
\DIFdelend \subsection{Possíveis Integrações e Parcerias}
    ...
    A rotina da clínica veterinária é constituída de processos \DIFdelbegin \DIFdel{, }\DIFdelend que organizam a rotina do estabelecimento, assim como seus procedimentos. \DIFdelbegin \DIFdel{Podemos }\DIFdelend \DIFaddbegin \DIFadd{Pode-se }\DIFaddend dizer que o cadastro de um cliente gera um fluxo de processo, relacionado a tal ação. A ordem em que um profissional realiza sua atividade, no caso \DIFdelbegin \DIFdel{do nosso projeto}\DIFdelend \DIFaddbegin \DIFadd{da CertVet}\DIFaddend , realiza uma consulta ou atendimento veterinário, segue-se um fluxo de ações, numa ordem \DIFdelbegin \DIFdel{pre-estabelecida}\DIFdelend \DIFaddbegin \DIFadd{pré-estabelecida}\DIFaddend , por convenção, pelos profissionais da área. 

    Um fluxo de processo garante ordenamento e agilidade, padronizando o processo para que todos os profissionais da área \DIFdelbegin \DIFdel{, ou do estabelecimento , }\DIFdelend \DIFaddbegin \DIFadd{e do estabelecimento }\DIFaddend tenham uma conduta uniforme, visando atingir melhores resultados.

    \DIFdelbegin \DIFdel{Baseado }\DIFdelend \DIFaddbegin \DIFadd{Com base }\DIFaddend na literatura e na experiência \DIFdelbegin \DIFdel{vivida pela integrante do grupo, }\DIFdelend \DIFaddbegin \DIFadd{prática da integrante Irina Chang, médica veterinária em atividade, }\DIFaddend foram elaborados os seguintes processos de cadastro e de atendimento de um animal \DIFdelbegin \DIFdel{, dentro de }\DIFdelend \DIFaddbegin \DIFadd{em }\DIFaddend uma clínica\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{. }\DIFaddend \citeonline{Rabelo:2012}.

    Ressalta-se que são processos do tipo \emph{AS-IS}, de um estabelecimento de médio porte, que \DIFdelbegin \DIFdel{nosso sistema }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend CertVet pretende englobar \DIFdelbegin \DIFdel{na sua versão futura, se possível no }\DIFdelend \DIFaddbegin \DIFadd{em seu }\DIFaddend produto final.
    ...
    \subsection{Contato Inicial - Necessidade de Atendimento}
     ...
     Neste fluxo um tutor necessita de atendimento para seu animal. Ele chega na recepção do estabelecimento e o seguinte processo é iniciado:
      ...
      - Caso não \DIFdelbegin \DIFdel{possui agendamento, verificar }\DIFdelend \DIFaddbegin \DIFadd{possua agendamento, recepcionista verifica }\DIFaddend disponibilidade de horários e \DIFdelbegin \DIFdel{informar }\DIFdelend \DIFaddbegin \DIFadd{informa ao }\DIFaddend tutor.

      - Tutor escolhe horário e aguarda atendimento.

      - Caso não exista o cadastro do tutor ou do animal, recepcionista \DIFdelbegin \DIFdel{deve coletar }\DIFdelend \DIFaddbegin \DIFadd{coleta }\DIFaddend os dados do tutor e animal para criar cadastro.

      - Após cadastro\DIFdelbegin \DIFdel{informar tutor da }\DIFdelend \DIFaddbegin \DIFadd{, recepcionista informa ao tutor sobre }\DIFaddend disponibilidade de horário.

      - Tutor escolhe horário e aguarda atendimento.

    O \DIFdelbegin \DIFdel{fim do processo se dá }\DIFdelend \DIFaddbegin \DIFadd{processo é finalizado }\DIFaddend com o animal sendo atendido pelo médico veterinário da clínica, caso o tutor tenha agendamento ou selecione um horário\DIFdelbegin \DIFdel{, ou o processo se encerra }\DIFdelend \DIFaddbegin \DIFadd{. O processo também pode ser encerrado }\DIFaddend caso não exista horário disponível ou tutor não aceite os horários \DIFdelbegin \DIFdel{ofertados}\DIFdelend \DIFaddbegin \DIFadd{oferecidos}\DIFaddend .
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            %DIF >  essas setas podem ser apagadas?
    ...
    \section{Análise de Requisitos}
        ...
        \subsection{Requisitos Funcionais}
            ...
            \begin{center}
                ...
                \hline
                RF11 & Gerar termos & O sistema deve permitir que \DIFdelbegin \DIFdel{os usuários gerem os }\DIFdelend \DIFaddbegin \DIFadd{o veterinário gere os }\DIFaddend termos citados no Quadro \ref{doc_obrigatorio_demanda} de Relação dos Documentos de Caráter Obrigatório e Demanda, conforme exigido pelo CFMV para exercício das clínicas veterinárias\DIFaddbegin \DIFadd{.}\DIFaddend \\
                \hline
        ...
        \subsection{Requisitos \DIFdelbegin \DIFdel{não }\DIFdelend \DIFaddbegin \DIFadd{Não }\DIFaddend Funcionais}
            ...
            \begin{center}
                ...
                %DIF >  \hline
                %DIF >  RNF08 & O sistema deve contar com um protocolo criptografado e seguro de transferência de dados em sua implementação. & Segurança\\
                \hline
                RNF08 & O sistema deve \DIFdelbegin \DIFdel{contar com um protocolo criptografado e seguro de transferência de dados em sua implementação. }%DIFDELCMD < & %%%
\DIFdel{Segurança}%DIFDELCMD < \\
%DIFDELCMD <                 \hline
%DIFDELCMD <                 %%%
\DIFdel{RNF09 }%DIFDELCMD < & %%%
...
\subsection{Histórias de Usuários}
    ...
    \begin{center}
        ...
        \hline
        %DIF >  REVER ESSA PRIMEIRA: deixar claro que usuario assina o documento fora da CertVet
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Diagrama Entidade-Relacionamento}}
                %DIFAUXCMD
%DIFDELCMD < \includegraphics[width=0.7\textwidth]{images/Modelo Lógico.v2.png}
%DIFDELCMD <                 %%%
\DIFdelendFL %DIF >      \begin{figure}[H]
        %DIF >          \centering
        %DIF >          \caption{Diagrama Entidade-Relacionamento}
        %DIF >          \includegraphics[width=0.7\textwidth]{images/Modelo Lógico.v2.png}

        \DIFdelbeginFL %DIFDELCMD < \label{fig:DER2}
%DIFDELCMD <                 \centering
%DIFDELCMD <         {\footnotesize %%%
\DIFdelFL{Fonte: Elaborado pelos autores.}%DIFDELCMD < }
%DIFDELCMD <             \end{figure}
%DIFDELCMD < %%%
\DIFdelend %DIF >          \label{fig:DER2}
        %DIF >          \centering
        ...
        {\footnotesize Fonte: Elaborado pelos autores.}
            ...
            \begin{figure}[H]
                \centering
                \caption{Diagrama Entidade-Relacionamento \DIFaddbeginFL \DIFaddFL{(Visão Ampliada)}\DIFaddendFL }
...
%DIFDELCMD <     %%%
\DIFdelend \section{Protótipos de Telas}   
    \DIFdelbegin \DIFdel{Para ter }\DIFdelend %DIF >  Para ter uma visão mais coerente do sistema, melhorar o entendimento de suas principais funcionalidades e descobrir pontos críticos ou que fujam do escopo do projeto foram desenvolvidos protótipos das telas iniciais do sistema.
\DIFaddbegin 

    \DIFadd{Com o objetivo de obter }\DIFaddend uma visão mais coerente do sistema, \DIFdelbegin \DIFdel{melhorar o entendimento }\DIFdelend \DIFaddbegin \DIFadd{aprimorar a compreensão }\DIFaddend de suas principais funcionalidades e \DIFdelbegin \DIFdel{descobrir }\DIFdelend \DIFaddbegin \DIFadd{identificar }\DIFaddend pontos críticos ou que \DIFdelbegin \DIFdel{fujam }\DIFdelend \DIFaddbegin \DIFadd{estejam além }\DIFaddend do escopo do projeto\DIFdelbegin \DIFdel{foram desenvolvidos protótipos das telas iniciais }\DIFdelend \DIFaddbegin \DIFadd{, foram desenvolvidas as telas de cada etapa }\DIFaddend do sistema.
    \DIFaddbegin 

    \DIFaddend Os protótipos ou \emph{wireframes} foram desenvolvidos em baixa e alta fidelidade, sem padrão de \emph{layout}, que será refinado no \emph{front-end} no decorrer do projeto.
...
\end{tabulary}
    ...
    \section{Arquitetura}
        ...
        \begin{figure}[H]
                ...
                \centering
                \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.65\textwidth]{images/ArquiteturaAplicacao_v7.png}
%DIFDELCMD <             \label{fig:infra4}
%DIFDELCMD <         %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.65\textwidth]{images/ArquiteturaAplicacao_v6.png}
                \label{fig:infra3}
    ...
    \section{Fases de entrega}
        ...
        O desenvolvimento do projeto foi dividido em três fases de entregas recorrentes, de forma que atendesse à demanda recebida por parte dos clientes. A primeira delas consiste na POC, a segunda a apresentação do MVP e, para finalizar, a Entrega Final. O Quadro \ref{tab:escopo} relaciona as principais funcionalidades da CertVet com a fase em que a sua implementação é esperada.
            ...
            \begin{quadro}[H]
                ...
                \DIFdelbegin %DIFDELCMD < \resizebox{\columnwidth}{!}{%
%DIFDELCMD <                 \begin{tabular}{|lrrrr|r|}
%DIFDELCMD <                     \hline
%DIFDELCMD <                     \multicolumn{1}{|l|}{Funcionalidades}                              & \multicolumn{1}{r|}{POC}   & \multicolumn{1}{r|}{MVP} & \multicolumn{1}{r|}{Produto Final}                                   \\ \hline\multicolumn{1}{|l|}{Processo de login}                  & \multicolumn{1}{c|}{X}  & \multicolumn{1}{c|}{X}   & \multicolumn{1}{c|}{X}                                 \\ \hline
%DIFDELCMD <                     \multicolumn{1}{|l|}{Módulo de autenticação}                  & \multicolumn{1}{c|}{}  & \multicolumn{1}{c|}{X}   & \multicolumn{1}{c|}{X}                                 \\ \hline
%DIFDELCMD <                     \multicolumn{1}{|l|}{Agendamento online}                         & \multicolumn{1}{c|}{} & \multicolumn{1}{c|}{X}   & \multicolumn{1}{c|}{X}                               \\ \hline
%DIFDELCMD <                     \multicolumn{1}{|l|}{Prontuário clínico digital}               & \multicolumn{1}{r|}{} & \multicolumn{1}{c|}{X}   & \multicolumn{1}{c|}{X}                             \\ \hline
%DIFDELCMD <                     \multicolumn{1}{|l|}{Registro de alterações assinados digitalmente} & \multicolumn{1}{r|}{}  & \multicolumn{1}{r|}{}   & \multicolumn{1}{c|}{X}                                 \\ \hline
%DIFDELCMD <                     \multicolumn{1}{|l|}{Gerenciamento de medicação}             & \multicolumn{1}{r|}{} & \multicolumn{1}{r|}{}   & \multicolumn{1}{c|}{X}                               \\ \hline
%DIFDELCMD <                     \multicolumn{1}{|l|}{Histórico de aspectos hereditários}                      & \multicolumn{1}{r|}{}      & \multicolumn{1}{r|}{}       & \multicolumn{1}{c|}{X}                           \\ \hline
%DIFDELCMD <                     \multicolumn{1}{|l|}{Controle de vacinação}                       & \multicolumn{1}{r|}{}      & \multicolumn{1}{r|}{}       & \multicolumn{1}{c|}{X}                           \\ \hline                  
%DIFDELCMD <                 \end{tabular}%
%DIFDELCMD <                 }
%DIFDELCMD <             %%%
\DIFdelend \DIFaddbegin \resizebox{\columnwidth}{!}{%
                \begin{tabular}{|lrrrr|r|}
                    \hline
...
\DIFdelFL{Versão da infraestrutura após as integrações com as APIs do IBGE e dos Correios}}
            %DIFAUXCMD
%DIFDELCMD < \centering
%DIFDELCMD <             \includegraphics[width=0.65\textwidth]{images/ArquiteturaAplicacao_v6.png}
%DIFDELCMD <             \label{fig:infra3}
%DIFDELCMD <         \end{figure}
%DIFDELCMD <     %%%
\DIFdelend %DIF >  \begin{figure}[H]
   %DIF >           \centering
   %DIF >           \caption{Versão da infraestrutura após as integrações com as APIs do IBGE e dos Correios}
   %DIF >           \centering
   %DIF >           \includegraphics[width=0.65\textwidth]{images/ArquiteturaAplicacao_v6.png}
   %DIF >           \label{fig:infra3}
   %DIF >   \end{figure}
    ...
    Entretanto, a consistência de disponibilidade do serviço não pode ser tomada como premissa, portanto \DIFdelbegin \DIFdel{é }\DIFdelend \DIFaddbegin \DIFadd{tornou-se }\DIFaddend necessário que uma solução independente da aplicação principal \DIFdelbegin \DIFdel{possa }\DIFdelend \DIFaddbegin \DIFadd{pudesse }\DIFaddend garantir que o banco de dados \DIFdelbegin \DIFdel{tenha }\DIFdelend \DIFaddbegin \DIFadd{dispusesse de }\DIFaddend tais informações disponíveis no momento da consulta pelos usuários ou que \DIFdelbegin \DIFdel{seja }\DIFdelend \DIFaddbegin \DIFadd{fosse }\DIFaddend capaz de enriquecer o banco de dados de forma automática posteriormente, notificando o usuário após conseguir recuperar o dado da fonte primária.
    \DIFdelbegin \DIFdel{Dessa forma, foi adicionada }\DIFdelend \DIFaddbegin \DIFadd{Para isso, a equipe resolveu adicionar }\DIFaddend à arquitetura\DIFaddbegin \DIFadd{, }\DIFaddend uma aplicação AWS Lambda que \DIFdelbegin \DIFdel{é }\DIFdelend \DIFaddbegin \DIFadd{fosse }\DIFaddend responsável por realizar a requisição ao serviço público e popular o banco de dados. Como opção de retaguarda para momentos de indisponibilidade do serviço (fallback), um banco de dados temporário \DIFdelbegin \DIFdel{é utilizado. Para essa solução}\DIFdelend \DIFaddbegin \DIFadd{seria utilizado. Logo}\DIFaddend , a equipe optou pelo banco de dados proprietário da AWS, DynamoDB\DIFdelbegin \DIFdel{. Esse banco será }\DIFdelend \DIFaddbegin \DIFadd{, que seria }\DIFaddend utilizado para dados efêmeros sobre requisições que não tiveram sucesso e que \DIFdelbegin \DIFdel{serão }\DIFdelend \DIFaddbegin \DIFadd{deveriam ser }\DIFaddend descartados após sucesso em nova tentativa. O esquema \DIFdelbegin \DIFdel{atual da arquitetura da CertVet }\DIFdelend \DIFaddbegin \DIFadd{desenhado pode ser visto na Figura \ref{fig:infra4}.
}

    \begin{figure}[H]
            \centering
            \caption{\DIFaddFL{Versão da infraestrutura após as integrações com as APIs do IBGE e dos Correios}}
            \centering
            \includegraphics[width=0.65\textwidth]{images/ArquiteturaAplicacao_v7.png}
            \label{fig:infra4}
        \end{figure}

    \DIFadd{No entanto, a API da ANVISA para requisição dos dados de medicamentos é muito instável e adicionava complexidade desnecessária para popular dados de medicamentos.
    Tornando necessário planejar uma aplicação dedicada para fornecer um serviço a parte que alimente o dado de forma automatizada. Como não é o foco da aplicação, a equipe optou por descartá-la da arquitetura nesse momento, podendo ser adicionado futuramente, caso a alimentação desses dados se torne uma necessidade estratégica de negócios. Por isso, a arquitetura aplicada no sistema passou a ser a desenhada anteriormente, como }\DIFaddend pode ser visto mais detalhadamente na Figura \DIFdelbegin \DIFdel{\ref{fig:infra4}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:infra3}}\DIFaddend .
    ...
    \DIFadd{Previamente, no subcapítulo \ref{subcap:tecnologia-backend} sobre as tecnologias aplicadas no desenolvimento do }\emph{\DIFadd{Back-end}}\DIFadd{, foi mapeado o uso do }\emph{\DIFadd{framework}} \DIFadd{Demoiselle para a assinatura de documentos em conformidade com a especificação ICP-Brasil. No entanto, a equipe optou por não utilizar o framework devido às dificuldades financeiras e de processos envolvidas na implementação de um certificado digital. Por outro lado, foi identificado um serviço web fornecido pelo órgão regulamentador, que é capaz de processar e reconhecer assinaturas compatíveis com o protocolo ICP-BR}\DIFaddend .
